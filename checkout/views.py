...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        if order_form.is_valid():
            ...
            """
                Getting the unique order PID to allow the user to order the same items again. Creating
                creating a new order instead of finding the previous order with the same items
            """
            pid = request.POST.get('client_secret').split('_secret')[0]
        ...
        if request.user.is_authenticated:
            """
                If the user exists and is logged into the application, then
                get the following information and pre populate the form with 
                that information 
            """
            try:
                ...
                """
                    If the user doesn't exist or is not logged into the application
...
def checkout_success(request, order_number):
    ...
    if request.user.is_authenticated:
        ...
        """
            If the user has selected to save there information to the profile

            Then get the information from the order and save it into the users profile
        ...
        if save_info:
            profile_data = {
                ...
                'default_street_address1': order.street_address2,
